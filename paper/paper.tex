...
to ensure that a physically realizable bicycle is obtained from the opimization
...
%
\begin{description}
  \item[Total $T$] The resulting combination of the five rigid bodies.
    \begin{itemize}
      ...
      \item The closed loop path tracking system~\cite{Hess2012} must be
        stable. This is required to obtain a valid HQM value.
    \end{itemize}
  ...
  \item[Wheels $F$, $R$] Both front and rear wheels have identical constraints.
    \begin{itemize}
      \item Wheel radius and mass must positive and be greater than a minimum
        value.
...
limited to 180 degrees. We introduce a constraint $c_{10}$ that prevents the
wheels from physically overlapping and require that the bicycle be lift-able by
